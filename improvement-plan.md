...
- ✅ **Matcher purity — decided + shipped (Aug 15):** non-product URLs are
  now excluded from the matcher entirely. Every candidate set — the exact
  tiers' `loadTheirsCandidates`, my own catalogue in `reconcilePair`, and
  the trigram recall tier's candidates — is filtered by the shared
  `hasJunkSegment` classifier (same single source of truth as the crawler
  and the ingest guard), UNLESS the doc carries a product identity
  (gtin/sku — a real product regardless of path shape, mirroring the
  ingest guard's rule exactly). Junk-to-junk fuzzy pairs (category/blog/
  SEO-city pages matching on shared tokens/grams) can no longer form.
  Remaining watch: legacy junk already paired into existing `ProductMatch`
  rows needs a reconcile re-run to clear (the next crawl of each pair does
  a full replace).
...
| **Worker code versioning** | ✅ **done (Aug 15).** The worker stamps `crawlerVersion` (backend package version + git short SHA at boot, best-effort) on the job at claim — first forced beat, opens the run log with `engine vX — restarting the backend deploys this` — exposed via `publicJob` and shown as an `engine <v>` chip on the Active crawls debug strip. A stale worker is now visible instead of a mystery. Live-verified: `1.0.0+eb33dfc` on a real job, matching the deployed commit. |
| **Failure classification** | ✅ **done (Aug 15).** `CrawlFailure.kind` (`'extraction'` = page loaded, nothing parsed; `'http'` = fetch failed — timeout/rate-limit/WAF/network) set at the two push sites + discovery failure. The run-log finish lines (engine AND worker) split the failed count (`[N extraction-miss · N http]`); the results panel badges each failure row (muted `no data` vs red `http`) and the empty state reads honestly ("every page loaded, nothing parseable" vs "some pages blocked"). Legacy results without `kind` render as before. |
