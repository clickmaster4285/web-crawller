...
export function DashboardPage() {
  ...
  if (isLoading) return <div>{header}<LoadingState /></div>;
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-px border-b border-border bg-border lg:grid-cols-4">
        <StatCard label="Competitor products" value={s.productsTracked.toLocaleString()} hint={`${s.competitors} sources`} />
        <StatCard label="Products matched" value={s.matchedProducts.toLocaleString()} hint="against your catalogue" />
        <StatCard label="Your products" value={s.yourProducts.toLocaleString()} hint="from your own store crawl" />
        <StatCard label="Only they sell" value={s.missingProducts.toLocaleString()} tone="accent" hint="catalogue gaps" />
        <StatCard label="Your average price" value={gbp(s.yourAvgPrice)} hint={`market ${gbp(s.marketAvgPrice)}`} />
        <StatCard label="Market average" value={gbp(s.marketAvgPrice)} hint="all matched competitors" />
        <StatCard label="Competitor out of stock" value={s.outOfStock.toLocaleString()} hint="demand you can capture" />
        <StatCard label="Cheapest competitor" value={s.cheapestCompetitor} hint={`priciest ${s.mostExpensiveCompetitor}`} />
      </div>

      <div className="grid gap-8 px-6 py-8 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <SectionTitle aside={<span className="text-xs text-muted-foreground">From price snapshots</span>}>
            Matched basket trend
          </SectionTitle>
          <div className="h-72 border border-border bg-card p-4">
            {data.priceHistory.length > 1 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data.priceHistory} margin={{ left: -18, right: 8, top: 8 }}>
        ...
        <div>
          <SectionTitle
            aside={
              <Link to="/pricing" className="text-xs text-accent underline underline-offset-4">
                Price intelligence
              </Link>
            }
          >
              ...
              .map((p) => (
                <li key={p.id} className="p-4">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {p.competitor} · {gbp(p.competitorPrice ?? 0)} vs your {gbp(p.yourPrice ?? 0)}
                  </p>
      ...
      <div className="border-t border-border px-6 py-8">
        ...
        <div className="h-72 border border-border bg-card p-4">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data.categoryGaps.slice(0, 10)} margin={{ left: -18, right: 8, top: 8 }}>
              <CartesianGrid stroke="var(--color-border)" vertical={false} />
              ...
              <Bar isAnimationActive={false} dataKey="yours" fill="var(--color-chart-1)" />
              <Bar isAnimationActive={false} dataKey="theirs" fill="var(--color-chart-5)" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="border-t border-border px-6 py-8">
        <SectionTitle
          aside={
            <Link to="/competitors" className="text-xs text-accent underline underline-offset-4">
              Manage sources
            </Link>
          }
        >
            ...
            <li key={c.id} className="flex items-center justify-between gap-4 p-4">
              <div className="min-w-0">
                ...
                <p className="truncate text-xs text-muted-foreground">
                  ...
                  {c.lastCrawlAt ? new Date(c.lastCrawlAt).toLocaleString() : "not crawled yet"}
                </p>
              </div>
