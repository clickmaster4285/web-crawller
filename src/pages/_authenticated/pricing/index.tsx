  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Price intelligence — Parity" },
      {
        ...
        content:
          "Daily price movement, volatility and the widest pricing gaps in your matched catalogue.",
      },
    ],
  }),
          ...
          <ul className="divide-y divide-border border border-border bg-card">
            {gaps.slice(0, 5).map((p) => (
              <li key={p.id} className="flex items-center justify-between gap-4 p-4">
                ...
                <span
                  className={cn("numeric text-sm", p.gap < 0 ? "text-destructive" : "text-success")}
                >
                  {p.gap > 0 ? "+" : ""}
                  {gbp(p.gap)}
                </span>
            ...
            {competitors.map((c) => (
              <li key={c.id} className="p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Badge
                    variant={c.avgPriceIndex < 100 ? "destructive" : "secondary"}
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {c.priceChanges} price changes in the last 24 hours · {c.frequency.toLowerCase()}{" "}
                  crawl
                </p>
                <div className="mt-3 h-1.5 w-full bg-muted">
                  <div
                    className={cn(
                      "h-full",
                      c.avgPriceIndex < 100 ? "bg-destructive" : "bg-success",
                    )}
