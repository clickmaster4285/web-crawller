...
import { brandGaps, categoryGaps, dashboardStats as s } from "@/lib/demo-data";
...
function Catalogue() {
  ...
  const sortedBrands = [...brandGaps].sort((a, b) => b.competitors - b.you - (a.competitors - a.you));
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-px border-b border-border bg-border lg:grid-cols-4">
        <StatCard label="Products only you sell" value={s.onlyYouSell.toString()} tone="positive" hint="unique assortment" />
        <StatCard label="Products only they sell" value={s.onlyTheySell.toLocaleString()} tone="accent" hint="addressable gap" />
        <StatCard label="Missing categories" value={s.missingCategories.toString()} hint="zero coverage" />
        <StatCard label="Missing brands" value={s.missingBrands.toString()} hint="stocked by rivals" />
      </div>
      ...
      <div className="px-6 py-8">
        ...
        <div className="h-80 border border-border bg-card p-4">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={sortedCategories} layout="vertical" margin={{ left: 60, right: 16, top: 8 }}>
